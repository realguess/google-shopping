...
#
fixme:
	@find \( -path './${DIR_NODE_MODULES}' -o -path './${DIR_GIT}' -o -name '${MAKEFILE}' \) -prune -o -type f -print0 | xargs -0 ${GREP} 'fixme\b'

#
# Show all todos.
#
todo:
	@find \( -path './${DIR_NODE_MODULES}' -o -path './${DIR_GIT}' -o -name '${MAKEFILE}' \) -prune -o -type f -print0 | xargs -0 ${GREP} 'todo\b'
